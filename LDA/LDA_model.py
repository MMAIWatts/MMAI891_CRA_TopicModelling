...
def coherence_vs_topics(method, texts, dictionary, corpus, min_number_topics=5, max_number_topics=15):
    scores = []
    topics_range = tqdm(range(min_number_topics, max_number_topics + 1))
    for n_topic in topics_range:
        model = topic_modeling(method = method, num_topics=n_topic, corpus=corpus, dictionary=dictionary)
    ...
    pd.DataFrame({'Number of Topics':trange, 'Coherence Score': scores}).to_csv(os.path.join(OUT_DIR, f'coherence_Topics_score_{method}.csv'))
    plt.figure(figsize = (6,4))
    plt.plot(trange, scores)
    plt.xticks(trange)
    plt.xlabel('Number of Topics')
    plt.ylabel('coherence score')
    plt.savefig(os.path.join(OUT_DIR, f'coherence_Topics_plot_{method}.png'))
    plt.show()
...
def sentences_chart(model, corpus, start=0, end=12):
...
def Plot_topic_dist(model, num_topics, corpus):
    topic_top3words = [(i, topic) for i, topics in model.show_topics(num_topics, num_words=20, formatted=False)
    ...
    df_dominant_topic_in_each_doc, df_topic_weightage_by_doc = topics_per_document(model, corpus, start=0, end=-1)
...
def word_count_topic(model, num_topics, texts):
...
# Wordcloud
def get_wordcloud(method , model, num_topics):
#     cols = [color for name, color in mcolors.TABLEAU_COLORS.items()]  # more colors: 'mcolors.XKCD_COLORS'
    ...
    topics = model.show_topics(num_topics=num_topics, num_words=50, formatted=False)
...
# t-SNE Clustering Chart
def tsne_plot(model, corpus, num_topics,
...
# pyLDAVis
def pyldavis_plot(model, corpus, mds="tsne"):
    pyLDAvis.enable_notebook()
    vis = pyLDAvis.gensim.prepare(model, corpus, dictionary=model.id2word, mds=mds)
    return vis


# Top 3 topics per document
def top3_topics_per_document(model, corpus):
    topics_top3 = []
